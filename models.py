...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, img_size, var=None):
        ...
        bs, num_y, num_x = p.shape[0], p.shape[-2], p.shape[-1]
        if (self.num_x, self.num_y) != (num_x, num_y):
            create_grids(self, img_size, (num_x, num_y), p.device, p.dtype)
        ...
        p = p.view(bs, self.num_anchors, self.num_classes + 5, self.num_y, self.num_x).permute(0, 1, 3, 4, 2).contiguous()  # prediction
        ...
        if self.training:
            ...
            return io.view(bs, -1, 5 + self.num_classes), p
...
def create_grids(self, img_size=416, ng=(13, 13), device='cpu', type=torch.float32):
    num_x, num_y = ng  # x and y grid size
    self.img_size = max(img_size)
    self.stride = self.img_size / max(ng)

    # build xy offsets
    yv, xv = torch.meshgrid([torch.arange(num_y), torch.arange(num_x)])
    self.grid_xy = torch.stack((xv, yv), 2).to(device).type(type).view((1, 1, num_y, num_x, 2))
