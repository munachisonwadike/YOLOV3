...
def train():
    ...
    model.hyp = hyper_param  # attach hyperparameters to model
    ...
    # print("\n\n\nSTART EPOCH", epoch_start," ", epochs," EPOCHS \n\n\n")
    # print("\n\n\nLENGTH", dataset.__len__(), "\n\n\n")

    # exit(1)
    for epoch in range(epoch_start, epochs):  # epoch ------------------------------------------------------------------
        model.train()
        ...
        if dataset.image_weights:
            ...
            image_weights = labels_to_image_weights(dataset.labels, nc=num_classes, class_weights=w)
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            sf = image_size / max(imgs.shape[2:])  # scale factor
            if sf != 1:
            ...
            s = ('%10s' * 2 + '%10.3g' * 6) % (
                '%g/%g' % (epoch, epochs - 1), '%.3gG' % mem, *mloss, len(targets), image_size)
    ...
    print('%g epochs completed in %.3f hours.\n' % (epoch - epoch_start + 1, (time.time() - t0) / 3600))
    ...
    if not args.evolve:  # Train normally
        ...
        for _ in range(1):  # generations to evolve
            if os.path.exists('evolve.txt'):  # if evolve.txt exists: select best hyps and mutate
                ...
                for i, k in enumerate(hyper_param.keys()):
                    hyper_param[k] = x[i + 7]
                ...
                for i, k in enumerate(hyper_param.keys()):
                    ...
                    hyper_param[k] *= float(x)  # vary by sigmas

            # Clip to limits
            keys = ['lr0', 'iou_t', 'momentum', 'weight_decay', 'hsv_s', 'hsv_v', 'translate', 'scale', 'fl_gamma']
            limits = [(1e-5, 1e-2), (0.00, 0.70), (0.60, 0.98), (0, 0.001), (0, .9), (0, .9), (0, .9), (0, .9), (0, 3)]
            for k, v in zip(keys, limits):
                hyper_param[k] = np.clip(hyper_param[k], v[0], v[1])
            ...
            print_mutation(hyper_param, results)